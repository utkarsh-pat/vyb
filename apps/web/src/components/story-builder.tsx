...
function initialMetadata(existing?: StoryOverlayMetadata | null): StoryOverlayMetadata {
...
function buildCompositionJson(metadata: StoryOverlayMetadata) {
  const composition = {
    ...
    media: {
      ...
      offsetX: metadata.transform.panX / CANVAS_WIDTH,
      offsetY: metadata.transform.panY / CANVAS_HEIGHT,
    },
    layers: [
      ...metadata.layers.map((layer) =>
        layer.kind === "text"
          ? {
              ...
              y: layer.y,
              color: layer.color,
              fontSize: layer.size / CANVAS_HEIGHT,
              align: layer.align,
              style: layer.style,
            }
          : {
      ...
      ...metadata.drawings.map((drawing) => ({
        ...
        id: drawing.id,
        color: drawing.color,
        width: drawing.width / CANVAS_WIDTH,
...
export function StoryBuilder({ asset, onApply, onClose }: StoryBuilderProps) {
  ...
  const currentRef = useRef(initialMetadata(asset.overlayMetadata));
  const [metadata, setMetadata] = useState(() => initialMetadata(asset.overlayMetadata));
  ...
  const renderCanvas = useCallback((exportMode = false) => {
    ...
    if (source) {
      ...
      if (sourceWidth > 0 && sourceHeight > 0) {
        ...
        context.save();
        context.translate(
          CANVAS_WIDTH / 2 + metadata.transform.panX,
          CANVAS_HEIGHT / 2 + metadata.transform.panY,
        );
    ...
    for (const layer of metadata.layers) {
      const x = layer.x * CANVAS_WIDTH;
      const y = layer.y * CANVAS_HEIGHT;
      context.save();
  ...
  }, [asset.kind, isPreview, metadata, selectedLayerId]);
  ...
  function findLayerAt(x: number, y: number) {
    return [...metadata.layers].reverse().find((layer) => {
      const layerX = layer.x * CANVAS_WIDTH;
      const layerY = layer.y * CANVAS_HEIGHT;
      ...
      setMetadata({
        ...
        drawings: [
          ...metadata.drawings,
          {
            ...
            color: drawColor,
            width: drawWidth,
            points: [{ x: point.x / CANVAS_WIDTH, y: point.y / CANVAS_HEIGHT }],
          },
        ],
      });
      ...
      setMetadata({
        ...
        layers: drag.initial.layers.map((layer) =>
          layer.id === drag.layerId
            ? {
                ...layer,
                x: Math.max(0.04, Math.min(0.96, layer.x + dx / CANVAS_WIDTH)),
                y: Math.max(0.04, Math.min(0.96, layer.y + dy / CANVAS_HEIGHT)),
              }
            : layer,
        ),
      });
        ...
        drawings: current.drawings.map((drawing) =>
          drawing.id === drag.drawingId
            ? {
                ...
                points: [
                  ...drawing.points,
                  { x: point.x / CANVAS_WIDTH, y: point.y / CANVAS_HEIGHT },
                ],
              }
            : drawing,
        ),
      ...
      <section className={`cs-story-builder${isPreview ? " is-preview" : ""}`}>
        ...
        <div className="cs-story-builder-body">
          <div className="cs-story-builder-stage-wrap">
            <canvas
              ...
              className="cs-story-builder-canvas"
            ...
            <aside className="cs-story-builder-controls">
              ...
              <fieldset>
                ...
                <div className="cs-story-builder-tool-row">
                  ...
                  <button type="button" className={metadata.transform.fit === "fill" ? "is-active" : ""} onClick={() => updateTransform({ fit: "fill" })}>Fill</button>
              ...
              <fieldset>
                ...
                <div className="cs-story-builder-stickers">
                  {STICKERS.map((sticker) => <button type="button" key={sticker} onClick={() => addSticker(sticker)}>{sticker}</button>)}
                </div>
              </fieldset>
        ...
        <footer className="cs-story-builder-footer">
          ...
          <button type="button" className="is-primary" onClick={() => void applyStory()} disabled={isExporting}>
            {isExporting ? "Exporting..." : asset.kind === "image" ? "Apply & flatten" : "Apply overlays"}
          </button>
        </footer>
      </section>
    </div>
